...
新版 Windows 脚本遇到 `agent.legacy-*` 状态迁移备份时，会在独立的 Windows PowerShell 进程中自动再次运行一次 `openclaw doctor --fix`。如果检测到 `OpenClaw Gateway` 计划任务动作中包含官方 `gateway.cmd` 或 `gateway.vbs`，但 `gateway.cmd` 已缺失，且 Gateway 已明确停止，脚本会提示使用官方 `openclaw gateway install --force` 重建启动器，并验证文件重新生成；证据不完整或重建失败时保留现有计划任务和配置。旧目录始终保留，不会被脚本删除。若独立进程仍无法确认服务归属，请关闭其他 OpenClaw/TUI/Gateway 窗口后，在新的 PowerShell 中重试 Doctor。
