...
| Windows PowerShell API-key input | Reads the API Key one console character at a time with masking and backspace support, avoiding the PowerShell 5.1/CMD `Read-Host -AsSecureString` truncation observed on affected machines | Passed by static review; Windows runtime pending |
